...
export default {
  ...
  methods: {
    ...
    async getGroups() {
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/getAllTeamsByGroupID?ids=` + this.filterIDs.toString());
    ...
    async getTimeslots() {
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/getAllTimeslotsByGroupID?ids=` + this.filterIDs.toString())
