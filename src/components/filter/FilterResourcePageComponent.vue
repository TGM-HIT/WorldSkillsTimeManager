...
export default {
  ...
  methods: {
    ...
    async getResources() {
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/getResourceByResourceID?ids=` + this.filterIDs.toString());
    ...
    async getTimeslots() {
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/getAllTeamsUsingResourceByID?ids=` + this.filterIDs.toString())
