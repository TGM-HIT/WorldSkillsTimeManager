      ...
      <v-col v-for="(item, index) in filterIDs" :key="index" cols="12" sm="6" md="4" lg="3">
        <v-card v-if="teams[index]" variant="outlined">
          ...
          <v-card-text style="color: #003866">
            <div v-if="teams[index].participants">
              participants: {{ teams[index].participants.toString() }}
            </div>
          </v-card-text>
        </v-card>
      </v-col>
  ...
  <v-container v-if="!loading">
    ...
    <v-row>
      <v-container v-for="(item, index) in timeslots.slice(0, 4)" :key="index" style="padding-bottom: 0px;">
        <v-card variant="outlined" v-if="timeslots[index].teamIDs">
          <v-card-title>
            <span style="color: #003866;">
              <strong>Time:</strong> <span style="color: #0E779F;"> {{ timeslots[index].time_from }} - {{ timeslots[index].time_to }}</span> <strong>Description: </strong><span style="color: #0E779F;">{{ timeslots[index].description }}</span> <strong>Resource:</strong><span style="color: #0E779F;">{{timeslots[index].resource}}</span>
            </span>
            <br>
            <span style="color: #003866;">
              <strong>Affected Team:</strong> <span style="color: #0E779F;">{{ getTeamNamesFromIDs(timeslots[index].teamIDs) }}</span>
            </span>
          </v-card-title>
        </v-card>
...
export default {
  ...
  methods: {
    ...
    getTeamNamesFromIDs(ids) {
      let names = "";
      for (let x = 0; x < ids.length; x++) {
        const index = this.filterIDs.indexOf(ids[x].toString());

        if (x === ids.length - 1) {
          names += this.teams[index].team_name
        } else {
          names += this.teams[index].team_name + "; ";
        }
      }
