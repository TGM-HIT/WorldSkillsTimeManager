...
export default {
  ...
  methods: {
    ...
    async setUpEdit(editId) {
      this.loading = true;
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/getRow?tablename=timeslottype&id=${editId}`);
    ...
    async createType() {
      if (this.type.name !== "" && this.type.description !== "") {
        ...
        try {
          ...
          const response = await axios.post(`${import.meta.env.VITE_API_URL}/api/setTable`, {
            table: 'timeslottype',
            data: {
              name: this.type.name,
    ...
    async editType(){
      if (this.type.name !== '' && this.type.description !== '' && this.type.color !== '') {
        ...
        try {
          const response = await axios.post(`${import.meta.env.VITE_API_URL}/api/updateTable`, {
            table: 'timeslottype',
            data: {
              id: this.editId,
              name: this.type.name,
