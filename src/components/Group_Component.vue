...
export default {
  ...
  methods: {
    async setUpEdit(editId) {
      this.loading = true;
  try {
    const groupResponse = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/getTable?tablename=groups`);
    ...
    if (groupResponse.data && groupResponse.data.length > 0) {
      ...
      if (groupData) {
        ...
        const groupTeamsResponse = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/getTable?tablename=groupteams`);
        ...
        try {
          ...
          if (groupId && this.group.teams.length > 0) {
            for (const teamId of this.group.teams) {
              await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/setTable`, {
                table: 'groupteams',
                data: {
                  groupid: groupId,
                  teamid: teamId
  ...
  if (this.group.name !== "" && this.group.teams.length !== 0) {
    ...
    try {
      ...
      await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/updateTable`, {
        table: 'groups',
        data: {
          id: this.editId,
          name: this.group.name
      ...
      await axios.delete(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/deleteRows`, {
        data: {
          table: 'groupteams',
    ...
    async GetValues() {
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/getUnasignedTeams`);
        if (response.data) {
